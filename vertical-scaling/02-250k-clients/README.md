# Kafkorama Gateway Benchmark - 250K Clients Test
...
```bash
ssh -A -i k-g-benchmark-key.pem admin@184.73.75.225
```
...
```bash
ssh -A -i k-g-benchmark-key.pem admin@54.234.235.65
```
...
```bash
ssh -A -i k-g-benchmark-key.pem admin@34.224.71.246
```
